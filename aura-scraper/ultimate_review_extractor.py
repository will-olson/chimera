#!/usr/bin/env python3
"""
Ultimate Review Extractor for Capterra
Uses advanced pattern matching and structural analysis to extract comprehensive review data
"""
...
class UltimateReviewExtractor:
    ...
    def extract_all_reviews(self) -> Dict[str, Any]:
        ...
        review_containers = self._find_all_review_containers()
        print(f"📊 Found {len(review_containers)} review containers")
        
        if not review_containers:
            return {"error": "No review containers found"}
        
        # Extract detailed data from each review
        extracted_reviews = []
        for i, container in enumerate(review_containers):
            review_data = self._extract_comprehensive_review(container, i)
            if review_data:
                extracted_reviews.append(review_data)
        
        return {
            "total_reviews": len(extracted_reviews),
            "reviews": extracted_reviews,
            "extraction_metadata": {
                "timestamp": datetime.now().isoformat(),
                "html_file": self.html_file,
                "method": "ultimate_comprehensive_extraction"
            }
        }
    ...
    def run_extraction(self) -> Dict[str, Any]:
        ...
        print("🚀 Starting ultimate review extraction...")
...
def main():
    """Main function to run the ultimate review extraction"""
    html_file = "Looker Features, Alternatives & More 2025 _ Capterra.html"
    
    extractor = UltimateReviewExtractor(html_file)
    results = extractor.run_extraction()
    
    # Save results
    output_file = f"output/ultimate_review_extraction_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
