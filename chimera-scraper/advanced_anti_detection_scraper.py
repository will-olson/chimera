#!/usr/bin/env python3
"""Advanced Anti-Detection Scraper with Proxy Rotation and Browser Automation."""
import asyncio
import json
import time
import random
...
class AdvancedAntiDetectionScraper:
    ...
    def __init__(self):
        ...
        # Free proxy list (for testing - in production use paid proxies)
        self.free_proxies = [
    ...
    async def test_with_browser_automation(self, url: str) -> Dict[str, Any]:
        """Test URL with browser automation (Playwright)."""
        ...
        try:
            ...
            async with async_playwright() as p:
                ...
                if response and response.status == 200:
                    ...
                    print(f"⚠️  Browser automation failed with status: {response.status if response else 'No response'}")
                    
                    # Try to get content even if status is not 200
                    try:
                        content = await page.content()
                        if "power bi" in content.lower() or "tableau" in content.lower() or "qlik" in content.lower():
                            print("✅ Content detected despite status code")
                            parsed_data = self.parser.parse_head_to_head_comparison(content, url)
                            await browser.close()
                            ...
                            return {
                                ...
                                "data": parsed_data,
                                "response_size": len(content),
                                "note": "Content extracted despite non-200 status"
                            }
                    except Exception as e:
    ...
    async def test_all_urls_advanced(self) -> List[Dict[str, Any]]:
        """Test all URLs with advanced anti-detection techniques."""
        print("🚀 Starting advanced anti-detection testing...")
    ...
    def export_advanced_test_results(self, output_dir: str = "output") -> str:
        """Export advanced test results."""
        Path(output_dir).mkdir(exist_ok=True)
        
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"advanced_anti_detection_results_{timestamp}.json"
    ...
    def get_advanced_test_summary(self) -> Dict[str, Any]:
        """Get summary of advanced anti-detection testing."""
...
async def main():
    """Main advanced anti-detection test execution."""
    print("🚀 Advanced Anti-Detection Scraper")
    print("=" * 50)
    
    scraper = AdvancedAntiDetectionScraper()
    
    try:
        # Test all URLs with advanced techniques
        results = await scraper.test_all_urls_advanced()
        
        # Export results
        output_file = scraper.export_advanced_test_results()
        
        # Print summary
        summary = scraper.get_advanced_test_summary()
        print("\n" + "="*60)
        print("📊 ADVANCED ANTI-DETECTION TESTING SUMMARY")
        print("="*60)
