"""Enhanced Capterra parser with advanced Cloudflare handling and anti-detection capabilities."""
...
class CapterraParser(BaseParser):
    """Enhanced Capterra parser with benchmark-level capabilities and Cloudflare bypass."""
    ...
    async def extract_reviews(self, html: str, source_url: str) -> List[EnhancedReview]:
        """Extract reviews with enhanced features and Cloudflare detection."""
        start_time = time.time()
        reviews = []
        
        try:
            # Check for Cloudflare protection
            if self._detect_cloudflare(html):
                self.cloudflare_detected = True
                logger.warning("Cloudflare protection detected in Capterra response")
                return []
            ...
            for element in review_elements:
                ...
                if review:
                    ...
                    self.update_extraction_stats("capterra_review_selector", True, extraction_time)
            ...
            logger.error(f"Error in enhanced Capterra review extraction: {e}")
            return []
    
    def _detect_cloudflare(self, html: str) -> bool:
        """Detect Cloudflare protection in HTML response."""
    ...
    def _extract_by_pattern_matching(self, soup: BeautifulSoup) -> List[Any]:
        ...
        all_elements = soup.find_all(['div', 'p', 'span', 'article'])
    ...
    async def _parse_single_review_enhanced(self, element: Any, source_url: str) -> Optional[EnhancedReview]:
        ...
        try:
            ...
            review = EnhancedReview(
                id=f"capterra_{hash(f'{author}_{date}_{text[:50]}')}",
                source="Capterra",
                ...
                extraction_method="enhanced_capterra_parser",
    ...
    def _extract_text_content(self, element: Any) -> str:
        ...
        text_selectors = [
            ...
            '.review-description', '.review-summary', '.review-details'
        ]
    ...
    def _extract_rating(self, element: Any) -> float:
        ...
        try:
            ...
            rating_selectors = [
                ...
                '.review__rating', '.review-item__rating', '.rating-stars'
            ]
            ...
            star_elements = element.select('[class*="star"], .star, [class*="rating"]')
            if star_elements:
                star_count = len([s for s in star_elements if 'filled' in s.get('class', []) or 'active' in s.get('class', [])])
                if star_count > 0:
    ...
    def _extract_date(self, element: Any) -> datetime:
        ...
        try:
            ...
            date_selectors = [
                ...
                '.review__date', '.review-item__date', '.review-date'
            ]
    ...
    def _extract_author(self, element: Any) -> str:
        ...
        try:
            ...
            author_selectors = [
                ...
                '.review__author', '.review-item__author', '.review-author'
            ]
            ...
            for line in lines:
                ...
                if line and len(line) < 50:
                    if not any(word in line.lower() for word in ['pros', 'cons', 'overall', 'experience', 'recommend']):
    ...
    def _parse_date_string(self, date_str: str) -> Optional[datetime]:
        ...
        try:
            ...
            date_formats = [
                ...
                "%Y-%m-%dT%H:%M:%S", "%Y-%m-%d %H:%M:%S", "%d %B %Y", "%B %d %Y"
            ]
    ...
    def _calculate_extraction_confidence(self, element: Any) -> float:
        ...
        if element.name in ['div', 'p', 'span', 'article']:
            confidence += 0.1
        
        # Check for review-like content
        text = element.get_text().lower()
        if any(phrase in text for phrase in ['pros', 'cons', 'like', 'dislike', 'experience', 'recommend']):
            confidence += 0.1
    ...
    def is_cloudflare_protected(self) -> bool:
        """Check if the last response was protected by Cloudflare."""
        return self.cloudflare_detected
